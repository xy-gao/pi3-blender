...
sys.path.append(os.fspath(Pi3_DIR))
...
class Dependencies:
    ...
    def install():
        ...
        if not os.path.exists(Pi3_DIR):
            try:
                subprocess.check_call(['git', 'clone', 'https://github.com/yyfz/Pi3.git', Pi3_DIR])
            except subprocess.CalledProcessError as e:
                print(f'Caught Exception while trying to git clone Pi3')
                print(f'  Exception: {e}')
                return False
    ...
    def check(*, force=False):
        ...
        if deps_path.exists() and os.path.exists(Pi3_DIR):
            try:
